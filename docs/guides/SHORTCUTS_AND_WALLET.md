...
如果“获取 URL 内容”的 JSON 请求体把完整字典放在名为 `交易信息` 的字段中，Worker 也会解包这一层并按同一规则导入；简体/繁体及 `transaction`、`transaction_info`、`transactionInfo` 包装名均兼容。推荐的新建快捷指令仍使用顶层字段，但现有嵌套快捷指令无需为此重做。
