...
- 趋势与分类分析同时返回 `net_spending_minor`、`net_income_minor` 和 `total_minor`：净支出为支出减退款，净收入为收入，总额是带符号的净现金流（收入为正、支出为负、退款为正）；转账和调整继续不计入这些指标。兼容字段 `amount_minor` 保持等于净支出。饼图用绝对值决定扇区大小，但提示金额必须保留原始正负号。
- 主页折线图与分析页分类饼图各自维护指标选择。主页切换会同步改变总览标题、汇总金额、上月同比和折线图；分析页切换会同步改变顶部币种汇总卡片的标题、金额、笔数、最大单笔、平均值，以及分类饼图。切换使用同一批 summary/trend 数据，不触发重复网络请求。
