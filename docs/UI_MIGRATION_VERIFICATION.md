...
- 用户明确要求主页折线图和分析页分类饼图支持“净支出 / 净收入 / 总额”切换；这是有意新增的图表交互，未改变页面路由、导航、卡片顺序、图表库、颜色或字体。主页总览标题、金额和同比会与所选折线指标同步。
- 两个图表各自维护选择状态，使用相同的三段式控件与 `aria-pressed`；主页选择器同步改变总览标题、汇总金额、上月同比和折线图，分类选择器只改变饼图及其说明。
...
- 首轮图表切换验证时浏览器扩展曾阻止访问本机 `127.0.0.1:5174`，因此按前端测试指南回退到项目 Playwright；本次汇总同步修复中浏览器连接成功，实测净支出 `HK$1,342.55`、净收入 `HK$0.00`、总额 `HK$1,342.55` 会随选择变化。截图通过运行时内存直接返回，没有写入或提交实例数据截图。
- Cloudflare 重新部署成功，远程 D1 无待执行 migration，首页、`/api/v1/health` 与 `/insights` SPA deep link smoke 全部通过。
- 用户随后明确“总额”是带符号现金流：收入为正、支出为负、退款为正。首页总额、折线和 API 使用带符号值；分类饼图以绝对值绘制扇区面积，并在 Tooltip 中保留正负号，避免负数扇区无法渲染。
- 本地浏览器实测首页总额为 `-HK$1,342.55` 且折线完整位于零线以下；分析页总额饼图显示 2 个支出扇区，悬停“未分类”显示 `-HK$1,264.15`，控制台无应用 error/warning。桌面与 iPhone 专项 Playwright 2/2 通过。
- 分析页顶部币种汇总卡片与分类饼图共用同一指标状态；选择净支出、净收入或总额时，卡片标题、金额、笔数、最大单笔和平均值同步切换，不新增第二个控件或网络请求。
- 浏览器实测同一 HKD 卡片依次显示净支出 `HK$1,342.55 / 54 笔`、净收入 `HK$0.00 / 0 笔`、总额 `-HK$1,342.55 / 54 笔`，总额平均值为 `-HK$24.86`；控制台无应用 error/warning。桌面与 iPhone 专项 Playwright 2/2 通过。
