...
现有快捷指令如果在“获取 URL 内容”的 JSON 请求体中显示“`交易信息` → `字典`”，无需重做。Worker 同时支持顶层字段和下面这种单层嵌套结构：
...
```json
{
  "交易信息": {
    ...
    "商户": "Example merchant",
    "卡片": "Example card",
    "时间": "2026-08-13T12:00:00+08:00"
  }
}
```

繁体包装名 `交易資訊`、`交易資料` 以及英文 `transaction`、`transaction_info`、`transactionInfo` 也受支持。Worker 只解包这一层已知字段；如果顶层同时提供标准字段，以顶层字段为准。

iOS Shortcuts 有时会把“字典”魔法变量放进 JSON 字段后再序列化为一段字符串。这种真机格式也无需重做快捷指令：Worker 会先将 `交易信息` 中的 JSON 字符串或 Shortcuts 键值文本解析成已知交易字段，然后再执行相同的验证、幂等和入账流程。空输入仍会返回 `amount: Invalid input`，因为没有金额的请求不能创建交易。
