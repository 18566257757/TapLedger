...
test('real Worker/D1 core flow: CRUD, review, analytics, automation, exports and logout', async ({ page, isMobile }) => {
  ...
  await expect(page.getByText('Signed cash flow by category: income positive, spending negative.')).toBeVisible()
  await page.reload()
  await expect(page.getByRole('heading', { name: 'Insights' })).toBeVisible()
...
test('mobile Cloudflare UI preserves navigation, editor interaction and width', async ({ page, isMobile }) => {
  ...
  const preferencesCard = await openSettingCard(page, 'Preferences')
  const lightMode = preferencesCard.getByRole('button', { name: 'Use light mode' })
  const darkMode = preferencesCard.getByRole('button', { name: 'Use dark mode' })
  ...
  await page.reload()
  await expect(page.locator('html')).toHaveAttribute('data-theme', 'light')
  const reloadedPreferencesCard = await openSettingCard(page, 'Preferences')
  await reloadedPreferencesCard.getByRole('button', { name: 'Use dark mode' }).click()
  await expect(page.locator('html')).toHaveAttribute('data-theme', 'dark')
  await expect(reloadedPreferencesCard.getByRole('button', { name: 'Use dark mode' })).toHaveAttribute('aria-pressed', 'true')
...
test('home line and category pie metrics switch without horizontal overflow', async ({ page }) => {
  ...
  const sectorBox = await signedSector.boundingBox()
  expect(sectorBox).not.toBeNull()
  await signedSector.hover({ force: true, position: { x: sectorBox!.width / 2, y: 5 } })
  await expect(categoryCard.locator('.chart-tooltip strong')).toContainText('-')
